...
def write_csv(tmp_path: Path, filename: str, rows: list[dict], fieldnames: list[str]) -> Path:
    """Write rows to a CSV. Rows use 'VALUE' as the value column key; it is remapped
    to fieldnames[1] so callers don't need to know the target column name."""
    p = tmp_path / filename
...
# STANDARD_ROWS uses generic 'VALUE' key — write_csv remaps to the actual column name
STANDARD_ROWS = [
...
class TestPageContentFormat:
    ...
    def test_no_fabricated_values(self, tmp_path):
        """Values in page_content must come from the CSV — not invented."""
        ...
        # Sanity: no value that was never in the CSV appears
...
class TestRowFiltering:
    ...
    def test_blank_date_rows_are_skipped(self, tmp_path):
        # First row must be valid so _detect_date_column can detect the date column.
        # The blank date is in a subsequent row — that row should be skipped.
...
class TestRealCSVFiles:
    """Smoke-validate the actual fred_fed_data CSVs without touching the DB."""
    ...
    def test_no_doc_has_empty_value(self, fred_data_dir):
        ...
        for doc in docs:
            # page_content ends with the value before the period
